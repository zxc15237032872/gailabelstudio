...
class Project(ProjectMixin, models.Model):
    ...
    def remove_tasks_by_file_uploads(self, file_upload_ids):
        # 记录警告日志，表明删除操作被禁用
        logger.warning('Task deletion by file upload IDs is disabled. No tasks will be removed.')
        return  # 直接返回，不执行删除操作
    ...
    def delete(self, using=None, keep_parents=False):
        """Override delete method to prevent project deletion"""
        logger.warning(f"Attempt to delete project {self.id} blocked: deletion is disabled")
        return None  # Return None to indicate deletion was not performed
    
    @classmethod
    def bulk_delete(cls, queryset, using=None):
        """Override bulk delete method to prevent multiple project deletions"""
        logger.warning(f"Attempt to bulk delete {queryset.count()} projects blocked: deletion is disabled")
        return 0, {}  # Return 0 deletions and empty dictionary
